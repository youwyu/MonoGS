	...
	const torch::Tensor& rgb_mask)
{
	...
	torch::Tensor binningBuffer = torch::empty({0}, options.device(device));
	torch::Tensor imgBuffer = torch::empty({0}, options.device(device));
	torch::Tensor psoBuffer = torch::empty({0}, options.device(device));
	std::function<char*(size_t)> geomFunc = resizeFunctional(geomBuffer);
	std::function<char*(size_t)> binningFunc = resizeFunctional(binningBuffer);
	std::function<char*(size_t)> imgFunc = resizeFunctional(imgBuffer);
	std::function<char*(size_t)> psoFunc = resizeFunctional(psoBuffer);

	torch::Tensor search_size = torch::full({6}, 0.18, float_opts);
	...
	if(P != 0)
	{
		...
		const int pso_idx = 45;

		rendered = CudaRasterizer::Rasterizer::forwardPso(
			geomFunc,
			binningFunc,
			imgFunc,
			psoFunc,
			...
			tan_fovy,
			prefiltered,
			pso.contiguous().data<float>(),
			pso_idx,
			search_size.contiguous().data<float>(),
			gt_depth.contiguous().data<float>(),
			depth_mask.contiguous().data<float>(),
			gt_image.contiguous().data<float>(),
			rgb_mask.contiguous().data<float>(),
			out_color.contiguous().data<float>(),
			out_depth.contiguous().data<float>(),
			out_opaticy.contiguous().data<float>(),
			radii.contiguous().data<int>(),
