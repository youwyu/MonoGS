...
class KittiParser:
    def __init__(self, input_folder, start_idx=0):
        ...
        if self.input_folder[-2:] == "00":
            self.length = 4541
        elif self.input_folder[-2:] == "04":
            self.length = 271

        self.color_paths, self.color_paths_r = [], []
        for ix in range(self.length):
            idx = '0'*(6-len(str(ix))) + str(ix)
            self.color_paths += [os.path.join(self.input_folder, 'image_2/'+idx+'.png')]
            self.color_paths_r += [os.path.join(self.input_folder, 'image_3/'+idx+'.png')]
        assert len(self.color_paths) == len(self.color_paths_r)
        self.color_paths = self.color_paths[start_idx:]
        self.color_paths_r = self.color_paths_r[start_idx:]
        self.n_img = len(self.color_paths)
        self.load_poses(os.path.join(self.input_folder, 'groundtruth.txt'))
    ...
    def load_poses(self, path):
        ...
        T_i_c0 = np.eye(4)
        for j in range(12):
            T_i_c0[j//4][j%4] = data[0][j]

        for ix in range(len(data)):
            T_w_i = np.eye(4)
            for j in range(12):
                T_w_i[j//4][j%4] = data[ix][j]
            T_w_c = np.dot(T_w_i, T_i_c0)
            T_c_w = np.linalg.inv(T_w_c)
            self.poses += [T_c_w]

            frame = {
                "file_path": self.color_paths[ix],
                "transform_matrix": (T_c_w).tolist(),
            }
...
class KittiDataset(StereoDataset):
    def __init__(self, args, path, config):
        super().__init__(args, path, config)
        dataset_path = config["Dataset"]["dataset_path"]
        parser = KittiParser(dataset_path, start_idx=config["Dataset"]["start_idx"])
        self.num_imgs = parser.n_img
        self.color_paths = parser.color_paths
        self.color_paths_r = parser.color_paths_r
        self.poses = parser.poses
