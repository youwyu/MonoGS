...
class GaussianModel:
    ...
    # cat the selected points to the current buffer
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        # randomly sample new 3D-Gaus in the original data
        new_xyz = torch.bmm(rots, samples.unsqueeze(-1)).squeeze(-1) + self.get_xyz[
            selected_pts_mask
        ].repeat(N, 1)
        # since the original scale is too large, scale it to 1 / 1.6
        ...
        # prune original 3D Gaus that have large scales
    ...
    # just clone the selected points
    ...
    # Adaptive Gaussian control uses partial derivative (dL/du) to determine which Gaussian to be split/cloned.
    # The function accumulates the gradient to every Gaussian in the visible frustum.
