...
__global__ void computeCov2DCUDA(int P,
								 ...
								 float *dL_dcov,
								 float *dL_dTcw,
								 const float* se3_Tcw_view,
								 const float* se3_Tcw_proj)
{
	...
	float3 t = transformPoint4x3(mean, se3_Tcw_view);
	...
	float3 dL_dmean = transformVec4x3Transpose({dL_dtx, dL_dty, dL_dtz}, se3_Tcw_view);
	...
	const float* se3_Tcw_proj)
{
	...
	float4 m_hom = transformPoint4x4(m, se3_Tcw_proj);
	float m_w = 1.0f / (m_hom.w + 0.0000001f);

	// Compute loss gradient w.r.t. 3D means due to gradients of 2D means
	// from rendering procedure
	glm::vec3 dL_dmean;
	float mul1 = (se3_Tcw_proj[0] * m.x + se3_Tcw_proj[4] * m.y + se3_Tcw_proj[8] * m.z + se3_Tcw_proj[12]) * m_w * m_w;
	float mul2 = (se3_Tcw_proj[1] * m.x + se3_Tcw_proj[5] * m.y + se3_Tcw_proj[9] * m.z + se3_Tcw_proj[13]) * m_w * m_w;
	dL_dmean.x = (se3_Tcw_proj[0] * m_w - se3_Tcw_proj[3] * mul1) * dL_dmean2D[idx].x + (se3_Tcw_proj[1] * m_w - se3_Tcw_proj[3] * mul2) * dL_dmean2D[idx].y;
	dL_dmean.y = (se3_Tcw_proj[4] * m_w - se3_Tcw_proj[7] * mul1) * dL_dmean2D[idx].x + (se3_Tcw_proj[5] * m_w - se3_Tcw_proj[7] * mul2) * dL_dmean2D[idx].y;
	dL_dmean.z = (se3_Tcw_proj[8] * m_w - se3_Tcw_proj[11] * mul1) * dL_dmean2D[idx].x + (se3_Tcw_proj[9] * m_w - se3_Tcw_proj[11] * mul2) * dL_dmean2D[idx].y;
	...
	float3 t = transformPoint4x3(m, se3_Tcw_view);
	// ---------------- Gradients w.r.t Tcw ------------
